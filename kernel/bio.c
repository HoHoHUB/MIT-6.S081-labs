...
binit(void)
{
  ...
  initlock(&bcache.hashlock, "bcache_hashlock"); // lab 8
...
//     b->prev = &bcache.head;
    initsleeplock(&b->lock, "buffer"); // lab 8
...
bget(uint dev, uint blockno)
{
  ...
  for (int i = 0; i < NBUCKET; ++i) {
    mintimestamp = -1;
    acquire(&bcache.locks[idx]);
    for (pre = &bcache.bucket[idx], b = pre->next; b; pre = b, b = b->next) {
    ...
    if (minb) {
        ...
        if (idx != HASH(blockno)) {
            ...
            idx = HASH(blockno);
            acquire(&bcache.locks[idx]);
            minb->next = bcache.bucket[idx].next;
            bcache.bucket[idx].next = minb;
        }
        release(&bcache.locks[idx]);
        release(&bcache.hashlock);
